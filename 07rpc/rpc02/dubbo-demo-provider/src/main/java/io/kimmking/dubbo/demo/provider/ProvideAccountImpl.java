...
import io.kimmking.dubbo.demo.api.mapper.*;
 ...
 * @version 1.0
 * @description: TODO
 * @date 2022年03月12日 19:03
 */
@Slf4j
@Component
public class ProvideAccountImpl implements BankAccountService {
    ...
    @Autowired
    BankFreezeMapper bankFreezeDao;
...
//        ConfirmLog confirmLog = confirmLogDao.selectById(tid);
//        confirmLogDao.
        //悬挂处理：如果cancel、confirm有一个已经执行了，try不再执行
        if (confirmLogDao.selectById(tid)!=null || cancelLogDao.selectById(tid)!=null) {
            log.info("bank1 中，tid 为 {} 的cancel 或者 confirm 操作已执行。直接退出。。", tid);
            return;
        }
        //扣减金额 冻结操作
        if (bankAccountDao.subtractAccountBalance(customerId, 2, amount)) {
        ...
        //调用consumerService 发起转账
        try {
            consumerService.transfer(tid,customerId, amount);
        } catch (Exception e) {
    ...
    public boolean confirm(String tid, int customerId, int amount) {
        ...
        if (confirmLogDao.selectById(tid)==null) {
            ...
            if (tryLogDao.selectById(tid)!=null && !(cancelLogDao.selectById(tid)!=null) ){
                //解除冻结金额
        ...
        if (cancelLogDao.selectById(tid)==null) {
            ...
            if (tryLogDao.selectById(tid)!=null) {
                ...
                if (confirmLogDao.selectById(tid)==null) {
                    ...
                    bankFreezeDao.subtractFreezeAmount(customerId, 2, amount);
                }
