...
import com.example.demo.db05.jdbc.JDBCUtil;
...
//        qry(ps,rs);
        ...
        for (int i = 0; i <1000000; i++) {
            ...
            System.out.println("插入执行结果:"+update1(ps,sqlStatement));
        }
...
//        System.out.println("删除执行结果:"+update(ps,sqlStatement));
    ...
    @DataSource(name = "db2")
    private static void qry2(PreparedStatement sta,ResultSet rs) throws SQLException {
        rs = sta.executeQuery();
        while(rs.next()) {
            System.out.println(rs.getObject("deptno"));
        }
     ...
     */
    @DataSource(name = "db1")
    private static int update1(PreparedStatement sta,String sql) throws SQLException {
        return sta.executeUpdate();
    }
    @DataSource(name = "db2")
    private static int update2(PreparedStatement sta,String sql) throws SQLException {
        return sta.executeUpdate();
    }
}
